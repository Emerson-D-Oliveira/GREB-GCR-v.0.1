...
function main ( args )
      ...
      _myname = 'lats4d: '
      _version = '2.1.6 of 26 January 2016'
...
* Load User Defined Extension table
* ---------------------------------
  if ( _udxt != '' )
     'load udxt ' _udxt
     say result
     if ( rc != 0 )
     	  say _myname 'could not load udxt: ' _udxt
          return rc
     endif
  endif
...
* Open alternate input file namea; must be conformant with
...
* -------------------------------------------------------
  ...
  if ( _bfname != '' )
      rc=openf(_bfname,_ftype)
      if ( rc != 0 )
         say _myname 'cannot open alternate file "'_bfname'"'
         return rc
      endif
      _nf = _nf + 1
      _bltf = _nf
...
* ----------------
   if ( _verb = 1 )
        ...
        if ( _afname != '' )
          'q file ' _altf
           say _myname 'First Alternate Data file is ' 
           say result
        endif
        if ( _bfname != '' )
          'q file ' _bltf
           say _myname 'Second Alternate Data file is ' 
           say result
        endif
   endif
...
function usage(flag)
      ...
      say ''
      say '     -udxt filename          Load User Dedined Extension table.'
      say '                             These extensions will be appended'
      say '                             to the system UDX table.'
      say ''
      say '     -v                      verbose mode'
      say ''
...
*
      ...
      options = '-model -center -table -udxt -v -q -i -j -k -o -format -vars -levs -time -ntimes -nlevels -mxtimes'
...
*                                 allows sdfopen templates
         ...
         if ( token = '-k' )
              ...
              first = x '' substr(token,1,1)
              while ( first != 'x-' & first != 'x' )
                _bfname = _bfname ' ' token
                 i = i + 1
                 token = subwrd(args,i)
                 first = x '' substr(token,1,1)
              endwhile
*                                 allows sdfopen templates
              if ( critique(1,3,'-k',_bfname) = 1 ); return 1; endif
         endif
         ...
         if ( token = '-udxt' )
              _udxt = subwrd(args,i+1) 
              i = i + 2
              token = subwrd(args,i)
              if ( critique(1,1,'-udxt',_udxt) = 1 ); return 1; endif
         endif
...
*        --------------------------------
         ...
         if ( token = '-geos0.5v' )
              rc = mkgeosf('geos0.5.ctl',0.625,0.5,576,361,'vt_p1')
              if ( rc != 0 ); return 1; endif
              i = i + 1
              token = subwrd(args,i)
         endif
